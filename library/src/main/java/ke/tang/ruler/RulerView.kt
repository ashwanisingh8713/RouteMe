...
//        markerHeight = markers.map { tempRect.apply { it.getBounds(this) }.height() }.max() ?: 0
        markerHeight = markers.map { tempRect.apply { it.getBounds(this) }.height() }.maxOrNull() ?: 0
        ...
        for (index in min(count, maxScaleCount) downTo minScaleCount) {
            ...
            if (labelRight > 0) {
                if (0 == index % sectionScaleCount || index == maxScaleCount || index == minScaleCount) {
                    ...
                    // Commented By Ashwani, to hide horizontal ruler text
